...
import { usePromptLikedBy } from '@/lib/hooks/usePromptLikedBy';
import UsersTable from '@/components/prompts/view/UsersTable';

export default function PromptLikedByPage() {
            ...
            <div className="flex items-center gap-4">
              <div className="text-center">
                <div className="flex items-center gap-2 text-accent">
                  <Icons.heart size={20} className="fill-accent" />
                  <span className="text-2xl font-bold">{prompt.likesCount || 0}</span>
                </div>
                <p className="text-xs text-secondary mt-1">Likes</p>
              </div>
            </div>
          </div>
        </div>

        {/* Users Who Liked Table */}
        <div className="bg-white rounded-lg border border-primary/10 overflow-hidden">
          <UsersTable
            users={likedByUsers.map(item => ({ user: item.user, timestamp: item.likedAt }))}
            loading={loadingLikes}
            title="Users Who Liked This Prompt"
            icon="heart"
