...
import { RatingFilter } from '@/lib/hooks/useFeedbackList';
...
interface SearchFilterBarProps {
  ...
  onRatingChange: (filter: RatingFilter) => void;
  onClearFilters: () => void;
}
    ...
    <div className="bg-white rounded-lg border border-primary/10 p-4">
      ...
      <div className="mt-4 pt-4 border-t border-primary/10">
        <p className="text-sm text-secondary font-body">
          ...
          <span className="font-semibold text-primary">{totalFeedback}</span> feedback
        </p>
      </div>
    </div>
  );
